...
import { Check, Globe, TrendingUp, Zap, Cpu, Sparkles, MessageCircle, ArrowRight } from 'lucide-react';
      ...
      <Reveal>
        <div className="text-center mb-14">
          ...
          <p className="text-base max-w-xl mx-auto" style={{ color: 'rgba(255,255,255,0.42)' }}>
            Chaque pack est pensé pour un stade précis de votre développement digital. Choisissez un pack pour préparer votre demande, sans paiement avant validation du besoin.
          </p>
        </div>
      </Reveal>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                  <div>
                    ...
                    <div className="space-y-2 mb-6">
                      {pack.benefits.map(b => (
                        <div key={b} className="flex items-center gap-2">
                          <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0" style={{ background: `${pack.color}18` }}>
                    ...
                    <div className="space-y-2">
                      <Link to={`/saas-contact?pack=${encodeURIComponent(pack.name)}`} className="block">
                      ...
                      <div className="flex gap-2">
                        <Link to="/saas-analyse" className="flex-1">
                          <motion.span whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.98 }}
                            className="w-full py-3 rounded-2xl font-black text-sm flex items-center justify-center"
                            style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.6)', border: '1px solid rgba(255,255,255,0.12)' }}>
                            Analyser mon projet
                          </motion.span>
                        </Link>
                        <a href={WA_LINK} target="_blank" rel="noopener noreferrer" aria-label={`Parler du ${pack.name} sur WhatsApp`}
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {pack.features.map(f => (
                        <div key={f} className="flex items-start gap-2 p-2.5 rounded-xl" style={{ background: 'rgba(255,255,255,0.02)' }}>
      ...
      <Reveal>
        <div className="max-w-2xl mx-auto text-center p-10 rounded-3xl relative overflow-hidden"
          ...
          <Link to="/saas-analyse">
            <motion.span whileHover={{ scale: 1.05, boxShadow: '0 0 50px rgba(212,175,55,0.45)' }} whileTap={{ scale: 0.97 }}
