...
export default function SaasLayout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-adn" style={{color:'white'}}>
      ...
      <footer className="relative z-10 border-t mt-16 py-12 px-4" style={{background:NOIR,borderColor:'rgba(212,175,55,0.12)',fontFamily:"'Poppins', sans-serif"}}><div className="max-w-7xl mx-auto"><div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10"><div className="col-span-2"><div className="flex items-center gap-3 mb-4"><PhoenixLogo size={40}/><div><div className="font-cinzel font-bold text-lg text-gold-gradient">Js-Innov.IA</div><div className="text-xs" style={{color:'rgba(212,175,55,0.35)'}}>Solutions Premium</div></div></div><p className="text-xs leading-relaxed" style={{color:'rgba(255,255,255,0.3)'}}>Votre problème devient notre point de départ.<br/>Votre solution devient notre création.</p></div><div><p className="text-xs font-bold tracking-widest uppercase mb-3" style={{color:GOLD}}>Navigation</p>{navItems.map(item=><Link key={item.path} to={item.path} className="block text-xs py-1" style={{color:'rgba(255,255,255,0.3)'}}>{item.label}</Link>)}</div><div><p className="text-xs font-bold tracking-widest uppercase mb-3" style={{color:GOLD}}>Services</p>{['Sites web','Studio créatif','Contenus digitaux','Automatisation','Agents IA'].map(s=><p key={s} className="text-xs py-1" style={{color:'rgba(255,255,255,0.3)'}}>{s}</p>)}</div></div><div className="border-t pt-6 text-xs" style={{borderColor:'rgba(212,175,55,0.08)',color:'rgba(255,255,255,0.18)'}}>© 2026 Js-Innov.IA · info@jsinnovia.com · 0494/11.90.90</div></div></footer>
